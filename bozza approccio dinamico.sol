...
contract GCDAO {
    ...
    uint32 public constant GeneralAssembly = 3075; // ID : 3 , control bitmask: 1100000
    ...
    uint8 public constant max_amount_of_rolesb = 32;
    uint8 roles_stored_amount = 7;
    mapping(address => uint8) public users_roles;
    address__OR__GCDAO_{{max_amount_of_rolesb / 2 }} previous_contract;
    uint8[ {{max_amount_of_rolesb }} ] role_permissioons;
    uint{{max_amount_of_rolesb }} [ {{max_amount_of_rolesb }} ] roleIndex_to_bitmask;
    mapping( string => uint8 ) public roleName_to_roleIndex;



    modifier controlledBy_index(uint8 target_role_index, address controller_role_address) {
       uint32 controller_role_id = roles[controller_role_address];
        ...
        uint{{max_amount_of_rolesb }} controller_role_bitmask = roleIndex_to_bitmask[ roles[controller_role_address] ];
        require( (controller_role_bitmask >> target_role_id) & 1 != 0, "the given controller can't perform the given operation on the given controlled one" );
