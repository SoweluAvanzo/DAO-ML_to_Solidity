...
contract Travelhive_DAO is IPermissionManager {
    ...
    function initializeCommittees(
        ...
        address _DAO_Council , 
        address _Marketing_Board , 
        address _Financial_Board , 
        address _Travelware_Board , 
        address _Development_Board 
        ) external {
        ...
        function canControl(
            uint32 controller,
            uint32 controlled
        ) public pure returns(bool controls){
        ...
        function assignRole(
            address _user,
            uint32 _role
        ) external controlledBy(msg.sender, roles[_user], true, _role) {
        ...
        function has_permission(address user, uint32 _permissionIndex) external view returns (bool) {
            return (role_permissions[uint32(roles[user] & 31 )] & (uint32(1) << _permissionIndex) != 0);
        }
