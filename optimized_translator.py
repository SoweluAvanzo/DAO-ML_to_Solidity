...
class SolidityOptimizedTranslationContext(TranslationContext):
    def __init__(self, dao: DAO, solidity_version="^0.8.0", daoOwner=True):
        ...
        # type_return_of(newEntityData)
        self.entity_to_data: dict[str, dict[str, any]] = {}


class OptimizedSolidityTranslator(Translator):
    def __init__(self, dao: DAO):
        self.context = SolidityOptimizedTranslationContext(
            dao, solidity_version="^0.8.0", daoOwner=True)
    ...
    def generate_modifier_control_relation_OLD(self):
        ...
        lines.append("\n")
        lines.append(
            f"    modifier controlledBy(address sender, {id_var_type} user_target_role_id, bool allowNullRole) {'{'}")
        lines.append(
        ...
        lines.append(
            f"            ( // \"CAN the sender control the target user (through its role)?\"")
        lines.append(
            f"                (allowNullRole && (user_target_role_id == 0)) || // allow to add role if the user has not already one assigned to it")
        lines.append(f"                ((")
        lines.append(
            f"                    (user_target_role_id >> {id_bit_size}) // get the role's bitmask ")
        lines.append(
            f"                    &  // (... and then perform the bitwise-and with ...)")
        lines.append(
            f"                    ( {id_var_type}(1) << ( roles[sender] & {mask} ) ) // sender_role_index")
        lines.append(f"                ) != 0) // THE FINAL CHECK")
        lines.append(f"            )")
        lines.append(
            f"            , \"the given controller can\'t perform the given operation on the given controlled one\" );")
        lines.append(f"            ")
        lines.append(f"            ")
        lines.append("        _;")
        lines.append("    }")
    ...
    def generate_modifier_control_relation(self):
        ...
        id_bit_size = self.group_size.value[1]
        mask = self.id_mask  # calcolato nel costruttore
        entities_amount = len(self.context.dao.roles) + \
            len(self.context.dao.committees)
    ...
    def generate_IPermissionManager_interface(self):
        ...
        lines.append(
            f"    function has_permission(address user, {self.perm_var_type} permissionIndex) external view returns (bool);")
        if self.context.dao.committees != {}:
            lines.append(
                f"    function canVote(address user, {self.perm_var_type} permissionIndex) external view returns (bool);")
            lines.append(
                f"    function canPropose(address user, {self.perm_var_type} permissionIndex) external view returns (bool);")
        lines.append("}")
    ...
    def translate(self) -> list[TranslatedSmartContract]:
        ...
        for c in self.context.dao.committees.values():
            ...
            translated_committee = ct.translateCommittee(
                c, proposal_permission_index, voting_permission_index, optimized=True)
            all_smart_contracts.append(translated_committee)
        all_smart_contracts.append(
            self.generate_IPermissionManager_interface())
        if self.context.dao.conditions != []:
            ...
            for condition in self.context.dao.conditions:
                if condition is not None:
                    ...
                    condition_sc = c.generate_condition_from_template("Templates/", condition_name=u.camel_case(
                        condition), condition_logic="//TODO: Implement the condition smart contract logic here", return_value="true", extension=".sol")
    ...
    def generate_contract_declaration(self):
        ...
        lines.append(
            f"contract {self.context.dao.dao_name} is IPermissionManager {'{'}")
        return "\n".join(lines)
    ...
    def generate_state_variables(self, visibility="internal", constant=""):
        ...
        lines = []
        lines.append(
            f"    bool {visibility} committee_initialization_blocked;")
        lines.append(
            f"    mapping(address => {id_var_type}) {visibility} roles;")
        if self.context.dao.voting_conditions != {}:
            lines.append(
                f"    mapping({id_var_type} => ICondition) {visibility} voting_conditions;")
        if self.context.dao.proposal_conditions != {}:
            lines.append(
                f"    mapping({id_var_type} => ICondition) {visibility} proposal_conditions;")
        if self.context.dao.assignment_conditions != {}:
            lines.append(
                f"    mapping({id_var_type} => ICondition) {visibility} assignment_conditions;")
        ...
        if is_role_access_optimized:
            ...
            total_roles_amount = len(
                self.context.dao.roles) + len(self.context.dao.committees)
            lines.append(
                f"    {self.perm_var_type}[{total_roles_amount} + 1] {visibility} role_permissions; // \"+1\" to allow the 0-index \"null Role\"")
        else:
            lines.append(
                f"    mapping({id_var_type} => {self.perm_var_type}) {visibility} role_permissions;")
        i = 0
        functionalities_ids = {}
        ...
        entities_amount = len(self.context.dao.roles) + \
            len(self.context.dao.committees)
        lines.append(
            f"    {id_var_type}[{entities_amount}] {visibility} all_roles = [")

        index_entity = 0
        for role in self.context.dao.roles.values():
            mask_shifted_for_id_bits, original_mask = self.get_control_bitflags(
                role, role.role_id, self.group_size, functionalities_ids)
            final_id = functionalities_ids[role.role_id] | mask_shifted_for_id_bits
            name_sanitized = role.role_name.replace(" ", "_")
            lines.append(
                f"        {final_id}{',' if index_entity != (entities_amount - 1) else ''} // #{index_entity}) {name_sanitized} -> ID : {functionalities_ids[role.role_id]} , control bitmask: {'{0:b}'.format(original_mask)}")
            index_entity += 1

            self.context.entity_to_data[role.role_id] = newEntityData(
                final_id=final_id, name=name_sanitized, index=index_entity, original_id=role.role_id, entity_type=EntityTypeControllable.ROLE)

        for committee in self.context.dao.committees.values():
            mask_shifted_for_id_bits, original_mask = self.get_control_bitflags(
                committee, committee.committee_id, self.group_size, functionalities_ids)
            ...
            lines.append(
                f"        {final_id}{',' if index_entity != (entities_amount - 1) else ''} // #{index_entity})  {name_sanitized} -> ID : {functionalities_ids[committee.committee_id]} , control bitmask: {'{0:b}'.format(original_mask)}")
            index_entity += 1
            self.context.entity_to_data[committee.committee_id] = newEntityData(
                final_id=final_id, name=name_sanitized, index=index_entity, original_id=committee.committee_id, entity_type=EntityTypeControllable.COMMITTEE)
    ...
    def generate_events(self):
        ...
        lines.append(
            f"    event RoleRevoked(address indexed user, {id_var_type} indexed role);")
        lines.append(
            f"    event RoleAssigned(address indexed user, {id_var_type} indexed role);")
        lines.append(
            f"    event PermissionGranted({id_var_type} indexed role, {self.perm_var_type} indexed permission);")
        lines.append(
            f"    event PermissionRevoked({id_var_type} indexed role, {self.perm_var_type} indexed permission);")
        return "\n".join(lines)
    ...
    def generate_constructor_v2(self):
        ...
        if self.context.daoOwner:
            lines.append(
                f"roles[msg.sender] = all_roles[{len(self.context.dao.roles)-1}]; // {self.context.dao.dao_name}Owner")
        if self.context.dao.conditions != []:
    ...
    def generate_constructor(self):
        committee_list_param = [f"address _{x}" for x in [committee.committee_description.replace(
            " ", "_")for committee in self.context.dao.committees.values()]]
        ...
        for committee in self.context.dao.committees.values():
            lines.append(
                f"         roles[_{committee.committee_description.replace(' ', '_')}] = {committee.committee_description.replace(' ', '_')}; \n")
        if self.context.daoOwner:
            # lines.append(f"        roles[msg.sender] = {self.context.dao.dao_name}Owner;")
            lines.append(
                f"        roles[msg.sender] = all_roles[{len(self.context.dao.roles)}]; // {self.context.dao.dao_name}Owner")
    ...
    def generate_committee_initialization_function(self, visibility="external"):
        ...
        lines = []
        lines.append(
            f"    function initializeCommittees({committee_address_list}) {visibility} {'{'}")
        index_role_owner = len(self.context.dao.roles) - 1
        lines.append(
            f"        require(roles[msg.sender] == all_roles[{index_role_owner}], \"Only the owner can initialize the Dao\");  // {self.context.dao.dao_name}Owner")
        if len(self.context.dao.committees) > 0:
            committee_requires = ' && '.join([f"_{x} != address(0)" for x in [committee.committee_description.replace(
                " ", "_") for committee in self.context.dao.committees.values()]])
            lines.append(
                f"    require(committee_initialization_blocked == false && {committee_requires}, \"Invalid committee initialization\");")

            committees_as_list = list(self.context.dao.committees.values())
            for index_committee, committee in enumerate(committees_as_list):
                sanitized_committee_name = committee.committee_description.replace(
                    " ", "_")
                lines.append(
                    f"        roles[_{sanitized_committee_name}] = all_roles[{index_committee}]; // {sanitized_committee_name}")
    ...
    def generate_role_permission_mapping(self):
        ...
        for role in self.context.dao.roles.values():
            entities_permissions.append((role.role_name.replace(" ", "_"), [self.get_permission_index(
                permission.permission_id) for permission in role.permissions]))
        for committee in self.context.dao.committees.values():
            entities_permissions.append((committee.committee_description.replace(" ", "_"), [
                                        self.get_permission_index(permission.permission_id) for permission in committee.permissions]))
        ...
        def role_to_index_fn(r, index_role): return f"{index_role}" \
            if is_role_access_optimized \
        ...
        for index_entity, entity_data in enumerate(entities_permissions, start=0):
            ...
            for index in permission_indices:
                bitflag |= (1 << index)
            lines.append(
                f"        role_permissions[{role_to_index_fn(role, index_entity)}] = {bitflag}; // #{index_entity}) {role} \n")
        return "\n".join(lines)
    ...
    def generate_user_role_condition_evaluation(self, condition_mapping_name, user_variable_name, roleInConditionCheckType: RoleInConditionCheckType = RoleInConditionCheckType.RETURN):
        def condition_fetching(
            is_generating_variable): return f"{'ICondition _condition =' if is_generating_variable else ''} {condition_mapping_name}[roles[msg.sender]] {';' if is_generating_variable else ''}"
        def expression_check(
            is_previously_fetched): return f"({'_condition' if is_previously_fetched else condition_fetching(False)} == ICondition(address(0))) || {'_condition' if is_previously_fetched else condition_fetching(False)}.evaluate({user_variable_name})"
        ...
        raise Exception(
            f"Invalid RoleInConditionCheckType: {roleInConditionCheckType}")
    ...
    def translate_DAO_to_ASM(self, dao: DAO) -> TranslatedSmartContract:
        ...
        entities_controllable: list[list[BaseEntity]] = [
...
class ConditionTranslator:
    ...
    def generate_condition_from_template(self, template_path: str, condition_name: str, condition_logic, return_value, extension=".sol") -> TranslatedSmartContract:
        ...
        file_name_and_path = template_path + \
            "ConditionImplementation" + extension + ".jinja"
        ...
        with open(file_name_and_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                rendered_line = template.render(solidity_version=self.context.solidity_version, condition_name=u.camel_case(
                    condition_name), condition_logic=condition_logic, return_value=return_value)
