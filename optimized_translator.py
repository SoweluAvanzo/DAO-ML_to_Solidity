...
class OptimizedSolidityTranslator(Translator):
    ...
    def generate_state_variables(self, visibility="internal", constant=""):
        ...
        if is_role_access_optimized:
            ...
            lines.append(
                f"    {self.perm_var_type}[{total_roles_amount}] {visibility} role_permissions;")
        else:
            lines.append(
        ...
        for role in self.context.dao.roles.values():
            ...
            self.context.entity_to_data[role.role_id] = newEntityData(
                final_id=final_id, name=name_sanitized, index=index_entity, original_id=role.role_id, entity_type=EntityTypeControllable.ROLE.value)
        ...
        for committee in self.context.dao.committees.values():
            ...
            self.context.entity_to_data[committee.committee_id] = newEntityData(
                final_id=final_id, name=name_sanitized, index=index_entity, original_id=committee.committee_id, entity_type=EntityTypeControllable.COMMITTEE.value)
    ...
    def generate_role_permission_mapping(self):
        ...
        is_role_access_optimized = self.group_size is not None
        def role_to_index_fn(
            irao, r, index_role): return f"{index_role}" if irao else r
        ...
        for index_entity, entity_data in enumerate(entities_permissions, start=0):
            ...
            for index in permission_indices:
                bitflag |= (1 << index)
            lines.append(
                f"        role_permissions[{role_to_index_fn(is_role_access_optimized, role, index_entity)}] = {bitflag}; // #{index_entity}) {role}")
        return "\n".join(lines)
