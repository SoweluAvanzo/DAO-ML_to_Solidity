...
class DiagramManager(base_entity_module.BaseEntity):
        ...
        for aggregated in role_or_committee.aggregated:
            ...
            for permission in aggregated.permissions:
                if permission not in role_or_committee.permissions:
                    source_id = role_or_committee.get_id() if isinstance(role_or_committee, role_module.Role) else role_or_committee.get_id()
                    target_id = aggregated.get_id() if isinstance(aggregated, role_module.Role) else aggregated.get_id()
    ...
    def processRawInstances(self):
        ...
        for daooo in self.daoByID.values():
            ...
            for committee in dao.committees.values():
                # Assign voting and proposal permissions to committees
                voting_permission = self.create_governance_permission(gp.GovernancePermission.VOTING, committee)
                proposal_permission = self.create_governance_permission(gp.GovernancePermission.PROPOSAL, committee)
    ...
    def generateOwnerRole(self, dao: dao_module.DAO):
        ...
        owner_role = role_module.Role(role_id = owner_role_name, role_name= owner_role_name, role_assignment_method = "Non Assignable", n_agent_min =None, n_agent_max=None, agent_type=None)
    ...
    def generate_conditions(self, dao: dao_module.DAO):
        ...
        for role in dao.roles.values():
            if role.role_assignment_method != None:
                dao.assignment_conditions[role.get_id()]= role.role_assignment_method
        ...
        for committee in dao.committees.values():
            if committee.voting_condition != None:
                dao.voting_conditions[committee.get_id()]= committee.voting_condition
                if committee.voting_condition not in conditions:
                    conditions.append(committee.voting_condition)
            if committee.voting_condition != None:
                dao.proposal_conditions[committee.get_id()]= committee.proposal_condition
