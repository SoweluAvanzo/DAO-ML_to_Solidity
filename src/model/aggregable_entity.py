...
class AggregableEntity(base_entity.BaseEntity):
    ...
    def add_permission(self, permission: permission.Permission):
        print(
            f'Adding permission {str(permission)} to {self.__class__.__name__} {self.id}')
    ...
    def add_committee_membership(self, target_committee):
        ...
        print(
            f'Adding {type(target_committee)} {target_committee.get_id()} to {self.__class__.__name__} {self.id}')
    ...
    def toJSON(self):
        ...
        obj["federated_committees"] = [c.get_id()
                                       for c in self.federated_committees]
        return obj
