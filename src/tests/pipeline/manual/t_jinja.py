...
JINJA_TEMPLATE_FILEPATH = files.concat_folder_filename(".", "Templates", "test_scripts", f"{JINJA_TEMPLATE_NAME}.{JINJA_TEMPLATE_EXTENSION}")

OUTPUT_COMPILED = files.concat_folder_filename(".", "outputs", f"{JINJA_TEMPLATE_NAME}.compiled")
...
if __name__ == "__main__":
    ...
    jinja_template_filepath = pstr.PIStr(pi.PIData(k_jinja_template_filepath, None), JINJA_TEMPLATE_FILEPATH)
    pm.addItem(jinja_template_filepath)

    k_jinja_template_input = "k_jinja_template_input"
    jinja_template_input = tfi.TextFileInput(pi.PIData(k_jinja_template_input, [k_jinja_template_filepath]))
    ...
    jinja_name = pstr.PIStr(pi.PIData(k_jinja_name, None), "TEST JINJA manuyally")
    ...
    compiled_printer = pri.PIPrinter(pi.PIData(k_compiled_printer, [k_jinja_template_compiler]), None, True)
    pm.addItem(compiled_printer)
    
    k_compiled_to_file = "k_compiled_to_file"
    compiled_to_file = tfo.TextFileOutput(pi.PIData(k_compiled_to_file, [k_jinja_template_compiler]), OUTPUT_COMPILED)
