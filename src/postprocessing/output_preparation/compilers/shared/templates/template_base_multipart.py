...
class TemplateBaseMultipart(tb.TemplateBase): 
    ...
    def get_all_parts_to_compile_as_generator(self, instance_data:dict, additional_data=None) -> Generator[crt.ConvertedSubpartTemplated, None, None]:
        raise Exception(f"ERROR: get_all_parts_to_compile_as_generator not implemented yet in {self.__class__.__name__}")
    
    def get_all_parts_to_compile(self, instance_data:dict, additional_data=None) -> list:
