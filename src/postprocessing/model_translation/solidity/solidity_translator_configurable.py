...
class SolidityTranslatorConfigurable(mcc.ModelTranslatorConfigurable):
    ...
    def get_converter_solidity_subtype(self, diagram: dm.DiagramManager, additional_data: dict = None) -> str:
        ...
        if converter_subtype is None:
            converter_subtype = self.get_default_converter_solidity_subtype(
                additional_data)
    ...
    def get_default_converter_version(self, converter_type: str, additional_data: dict = None) -> str:
        converter_solidity_subtype = self.get_default_converter_solidity_subtype(
            additional_data)
        if converter_solidity_subtype == tts.TranslationTypesSolidity.OPTIMIZED.value:
            ...
            if additional_data is not None:
                additional_data[SolidityTranslatorConfigurable.__KEY__SOLIDITY_CONVERTER_OPTIMIZED] = sco
    ...
    def select_implementation(self, diagram: dm.DiagramManager, converter_type: str, converter_version: str, converter_target: str, additional_data: dict = None) -> mcb.ModelTranslatorBase:
        impl = None
        converter_solidity_subtype = self.get_converter_solidity_subtype(
        ...
        match converter_solidity_subtype:
            case tts.TranslationTypesSolidity.OPTIMIZED.value:
                ...
                if additional_data is not None and SolidityTranslatorConfigurable.__KEY__SOLIDITY_CONVERTER_OPTIMIZED in additional_data:
                    sco_instance = additional_data[SolidityTranslatorConfigurable.__KEY__SOLIDITY_CONVERTER_OPTIMIZED]
                else:
                    sco_instance = self.new_solidity_converter_optimized(
                        additional_data)
