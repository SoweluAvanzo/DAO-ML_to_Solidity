...
class SolidityConverterOptimizedConfigurable(stc.SolidityConverterConfigurable):
    ...
    def new_subclass_instance(self, converter_version:str, additional_data:dict=None):
        ...
        match converter_version:
            case j_o_v.JinjaOptimizedVersions.JO_1_0_0.value:
                converter_instance = coj_1_0_0.SolidityConverterOptimizedJinja_1_0_0(self.get_pipeline_item_data(), \
                        self.key_model, \
    ...
    """
        raise Exception(f"(Solidity)SolidityConverterOptimized can't select its own delegator for translator type ({translator_type}) and version ({version}): instantiate another one.")
    """
