...
class TranslatorOptimized(stg.TranslatorGeneral):
    ...
    def select_delegator(self, diagram:dm.DiagramManager, translator_type:str, version:str,  additional_data:dict=None) -> mcb.ModelConverterBase:
        # TODO selezionare se jinja od altro, 
        # basic implementation
        # ... 12-07-2025: given the current packages tree, we assume that "translator_type" is, somehow, "Jinja"
        if version == j_o_v.JinjaOptimizedVersions.JO_1_0_0.value:
            return to_sol_j_1_0_0.TranslatorOptimizedJinja_1_0_0(self.get_pipeline_item_data(), \
                                                                ...
                                                                self.key_version)
            # TODO: SHALL WE MOVE THIS INSTANTIATION INTO "TranslatorOptimizedJinja"?
    ...
    def translate(self, diagram:dm.DiagramManager, additional_data:dict=None) -> stg.TranslatedDiagram:
        ...
        # get the delegator
        deleg = self.select_delegator(diagram, None, translator_version, additional_data)
        if deleg is None:
            return None
