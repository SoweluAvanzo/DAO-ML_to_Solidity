...
class AntlrInvoker(cli_module.CLIExecutor):
    def __init__(self, pipeline_item_data: pi.PIData, input_key_grammar_config: str,
        ...
        if input_key_grammar_config is None:
            raise Exception(
                "AntlrInvoker needs a non-None, non-empty 'input_key_grammar_config' construct parameter")
    ...
    def get_grammar_configurations_from_inputs(self, inputs: dict):
        ...
        if grammar_config is None or not isinstance(grammar_config, AntlrConfig):
            raise Exception(
                f"ERROR: can't run {type(self)} with a grammar_config (from input) of the wrong type: expected AntlrConfig, got: {type(grammar_config)}")
        return grammar_config

    def commands_froms_inputs(self, inputs):
        try:
            antlr_path = file_utils.concat_folder_filename(
                consts.EXTERNAL_LIBS_FOLDER, f"antlr-{self.antlr_version}.jar")
            grammar_config: AntlrConfig = self.get_grammar_configurations_from_inputs(
                inputs)
            folder_path = grammar_config.folder_path
            grammar_filepath = grammar_config.get_grammar_filepath(
                self.file_extension)
            ...
            lexer_filepath = grammar_config.get_lexer_filepath(
                self.file_extension)  # grammar_filepath
            to_compile_data = [
            ...
            path_init_file: str = file_utils.concat_folder_filename(
        ...
        for f in files_in_compilation_folder:
            ...
            if extension_index >= 0:
                ...
                if extension in set_extensions \
                    ...
                    file_path_to_remove = file_utils.concat_folder_filename(
                        folder_path, f)
    ...
    def execute_command(self, command: str, inputs: dict, index: int = None):
        try:
            ...
            print(
                "ERROR while remove_previous_compilation_outputs during execute_command:")
            print(e)
