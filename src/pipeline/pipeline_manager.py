...
class PipelineNode(pi.PipelineItem):
    ...
    def addDependant(self, newDependant: any):
        if not isinstance(newDependant, PipelineNode):
            raise TypeError(
                f"Provided dependant is not a PipelineNode, but a {type(newDependant)}")
        ...
        newDependant.dependency_inputs[self.item.get_key(
        )] = newEmptyDependencyOutputData()
    ...
    def canRun(self):
        ...
        for dep in self.item.get_dependencies():
            # are all dependencies satisfied? if not -> can't run
            if (dep not in self.dependency_inputs) or (not self.dependency_inputs[dep][0]):
                return False
        return True
    ...
    def addInput(self, input_value: any, dependency_job: pi.PipelineItem):
        ...
        output_data = None
        # if the data is already present, then update the "completed" value (the boolean value, first item of the array)
        if key in self.dependency_inputs:
...
class PipelineManager:
    ...
    def runPipeline(self) -> any:
        ...
        # a.k.a. "frontier" ; single thread -> only one item at a time can run
        job_queues = deque(roots)
        del roots
        # RUN
        final_outputs_by_key: dict = {}
        while len(job_queues) > 0:
            ...
            try:
                ...
                if len(job.getDependants()) > 0:
                    ...
                    final_outputs_by_key[job.get_key()] = output
            except Exception as e:
                job.status_run = NodeRunStatus.CRASHED
                print(
                    f"ERROR EXCEPTION while running job: {job.item.get_key()}")
                print(e)
                import traceback
                ...
                job.status_run = NodeRunStatus.CRASHED
                print(
                    f"ERROR TYPE-ERROR while running job: {job.item.get_key()}")
                print(e)
                import traceback
                traceback.print_exception(e)
