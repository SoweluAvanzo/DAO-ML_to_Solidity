...
class ConstraintValidator():
    ...
    def validate_against_schema(self):
        ...
        try:
            ...
            if validation.is_valid(diagram_xml):
                ...
                error_output = []
                print(
                    f'The XML file is not valid. Found {len(errors)} error(s):')
                for error in errors:
                    # append error name
        ...
        for elem in diagram.xpath("//Role | //Committee"):
            ...
            if relation:
                for relator_id in relation:
                    ...
                    relator_elems = diagram.xpath(
                        f'//*[@role_ID="{relator_id}"] | //*[@committee_ID="{relator_id}"]')
    ...
    def check_relations_in_same_DAO(self, diagram, early_return=False):
        ...
        all_descendants_name_list = " | ".join(
            [f"./{dn}/text()" for dn in all_descendants_name])
        ...
        for dao in diagram.xpath('//DAO'):
            ...
            for elem in dao.xpath("./Role | ./Committee"):
                ...
                possible_targets[elem_id] = set(
                    [target for target in elem.xpath(all_descendants_name_list)])

            for elem in dao.xpath("./Permission"):
                elem_id = elem.get('permission_ID')
                target = elem.get('ref_gov_area')
                # each permission has only one single target/reletaion
                possible_targets[elem_id] = set([target])
        ...
        for dao_id in possible_targets_by_dao_id:
            ...
            for element_id in elements_in_dao:  # for each element inside a Dao
                # TODO: CHECK IF IT's A SET
                element = elements_in_dao[element_id]
    ...
    def check_cyclic_dependencies(self, diagram, rel_name):
        ...
        for dao in diagram.xpath('//DAO'):
            ...
            for elem in dao.xpath("./Role | ./Committee"):
                ...
                id = elem.get("role_ID") if is_role else elem.get(
                    "committee_ID")
    ...
    def validate_dao_ml_diagram(self):
        ...
        conditions.append(self.compare_subsets(diagram, "IDs that roles or committees are associated to",
                          '//Role/associated_to/text()| //Committee/associated_to/text()', "Permission or Governance Area IDs", '//Permission/@permission_ID'))
        conditions.append(self.compare_subsets(diagram, "IDs of elements that roles aggregate into",
                          '//Role/aggregates/text()', "Roles' IDs", '//Role/@role_ID'))
        conditions.append(self.compare_subsets(diagram, "IDs of elements that committees aggregate into",
                          '//Committee/aggregates/text()', "Committee' IDs", '//Committee/@committee_ID'))
        conditions.append(self.compare_subsets(diagram, "IDs of control relations", '//Role/is_controlled_by/text()|//Committee/is_controlled_by/text()',
                          "Role or Committee IDs", '//Role/@role_ID|//Committee/@committee_ID'))
        conditions.append(self.check_relation_graphs(
            diagram, "aggregation_level", "aggregates"))
        conditions.append(self.check_relation_graphs(
            diagram, "federation_level", "federates_into"))
        conditions.append(
            self.check_cyclic_dependencies(diagram, "aggregates"))
        conditions.append(self.check_cyclic_dependencies(
            diagram, "federates_into"))
        conditions.append(self.check_relations_in_same_DAO(
            diagram, early_return=False))
        i = 0
