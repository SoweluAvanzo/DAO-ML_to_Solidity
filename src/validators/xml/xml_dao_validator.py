...
class ConstraintValidator():
    ...
    def validate_dao_ml_diagram(self, diagram=None):
        ...
        conditions.append(lambda s, d: s.compare_subsets(d, "IDs that roles or committees are associated to",
                          '//Role/associated_to/text()| //Committee/associated_to/text()', "Permission or Governance Area IDs", '//Permission/@permission_ID'))
        conditions.append(lambda s, d: s.compare_subsets(
            d, "IDs of elements that roles aggregate into", '//Role/aggregates/text()', "Roles' IDs", '//Role/@role_ID'))
        conditions.append(lambda s, d: s.compare_subsets(d, "IDs of elements that committees aggregate into",
                          '//Committee/aggregates/text()', "Committee' IDs", '//Committee/@committee_ID'))
        conditions.append(lambda s, d: s.compare_subsets(d, "IDs of control relations",
                          '//Role/is_controlled_by/text()|//Committee/is_controlled_by/text()', "Role or Committee IDs", '//Role/@role_ID|//Committee/@committee_ID'))
        conditions.append(lambda s, d: s.check_relation_graphs(
            d, "aggregation_level", "aggregates"))
        conditions.append(lambda s, d: s.check_relation_graphs(
            d, "federation_level", "federates_into"))
        conditions.append(
            lambda s, d: s.check_cyclic_dependencies(d, "aggregates"))
        conditions.append(
            lambda s, d: s.check_cyclic_dependencies(d, "federates_into"))
        conditions.append(
            lambda s, d: s.check_relations_in_same_DAO(d, early_return=False))
        i = 0
        error_texts = []
